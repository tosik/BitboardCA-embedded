...
LargeBitboard::LargeBitboard(unsigned int size_x, unsigned int size_y,
                             bool direct_size_mode) {
  m_SizeX = size_x;
  m_SizeY = size_y;
...
LargeBitboard::~LargeBitboard() { delete[] m_BitboardList; }

Bitboard *LargeBitboard::GetBitboardList() const { return m_BitboardList; }

inline Bitboard LargeBitboard::GetBoard(unsigned int x, unsigned int y) const {
  return m_BitboardList[x + y * 8];
}

unsigned int LargeBitboard::GetSizeX() const { return m_SizeX; }

unsigned int LargeBitboard::GetSizeY() const { return m_SizeY; }

void LargeBitboard::Randomize(int seed) {
  std::mt19937 mt(seed);
  for (unsigned int i = 0; i < GetBitboardListSize(); i++)
    // generate random number of 64 bit
    m_BitboardList[i] =
        mt() + mt() * 0x0000000000010000ULL + mt() * 0x0000001000000000ULL +
        mt() * 0x0100000000000000ULL + (mt() % 0xf) * 0x1000000000000000ULL;
}

void LargeBitboard::Clear() { Clear(m_BitboardList, GetBitboardListSize()); }
...
inline unsigned int LargeBitboard::GetBitboardIndex(unsigned int x,
                                                    unsigned int y) const {
  return x / 8 + y / 8 * m_BitboardSizeX + 1 + m_BitboardSizeX;
}
inline unsigned int LargeBitboard::GetBitboardShiftSize(unsigned int x,
                                                        unsigned int y) const {
  return (7 - x % 8) + (7 - y % 8) * 8;
}

unsigned int LargeBitboard::GetCellState(unsigned int x, unsigned int y) const {
...
void LargeBitboard::SetCellState(bool cell, unsigned int x, unsigned int y) {
  ...
  if (cell)
    ...
    m_BitboardList[bitboard_index] &=
        (Bitboard)0xffffffffffffffffULL ^ ((Bitboard)1 << shift_size);
}

Bitboard LargeBitboard::GetBitboard(unsigned int x, unsigned int y) const {
  return m_BitboardList[x + y * m_BitboardSizeX];
}

Bitboard LargeBitboard::GetBitboard(unsigned int i) const {
...
unsigned int LargeBitboard::GetBitboardListSize() const {
  return m_BitboardSizeX * m_BitboardSizeY;
}

unsigned int LargeBitboard::GetBitboardListSizeX() const {
  return m_BitboardSizeX;
}

unsigned int LargeBitboard::GetBitboardListSizeY() const {
